	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="entry-content">
			<figure class="wp-block-image">
				...
				$image_size = apply_filters( 'seges_one_attachment_size', 'full' );
			...
			wp_link_pages(
				array(
					'before'   => '<nav class="page-links" aria-label="' . esc_attr__( 'Page', 'segesone' ) . '">',
					'after'    => '</nav>',
					/* translators: %: Page number. */
					'pagelink' => esc_html__( 'Page %', 'segesone' ),
				)
			);
			?>
		</div>
		...
		<footer class="entry-footer default-max-width">
			...
			if ( wp_get_post_parent_id( $post ) ) {
				...
				printf(
					...
					esc_html__( 'Published in %s', 'segesone' ),
			...
			if ( $metadata ) {
				printf(
					...
					esc_html_x( 'Full size', 'Used before full size attachment link.', 'segesone' ), 
